...
from dataloader import loading_datasets
...
parser.add_argument('--ps', type=int, default=160, help='patch size')
...
def pixel_shuffle(x, sz, sr_rec, f):
    ...
    if sr_rec: # x:[B*ids*ids, C, H/ids, W/ids]  sz=torch.Shape([H/ids, W/ids])
        ...
        x = x.view(B, f*f, C, H, W) # [B, ids*ids, C,H,W]
...
''' x [ids,ids] '''
# import itertools
# rows_combs = list(itertools.combinations(range(args.tds), args.ids))   # C(p,s)
# cols_combs = list(itertools.combinations(range(args.tds), args.ids))   # C(p,s)
...
# candidate_idxsets = np.array(candidate_idxsets, dtype=np.int64) # (C(p,s)*C(p,s), s*s)i64 
...
def generate_mask_pair(img):  # img:[n,c,h,w]f32[0,1]
    ...
    num_blocks = n_batch * (h // p) * (w // p)
    # rd_idx: (num_blocks, p2)
    ...
    for blk in range(num_blocks): # nhw/p^2
        ...
        selected_idx_list = []
        used_mask = np.zeros(p2, dtype=bool) # (p*p,)
        for cand_idx in cand_indices_master:
            cand_set = candidate_idxsets[cand_idx] # (s*s,)
        ...
        front_idx = np.concatenate(selected_idx_list).astype(np.int64) # req_n * s*s
        ...
        arr = np.concatenate((front_idx, tail)) # p2
        rd_idx[blk, :] = arr
...
def generate_imgs_pair(img, masks): # img:[b,c,h,w]  masks:[p^2][nhw]  only nhw/p^2 ones in each row[nhw]
    ...
    subimages = torch.zeros(
        b, len(masks), c, h // p, w // p, 
        dtype=img.dtype, device=img.device) # [b, p^2, c, h/p, w/p]f32
    
    for i in range(len(masks)): # [p^2]
        for j in range(c):
            img_per_ch = F.unfold(img[:, j:j+1, :, :], p, stride=p) # [b,1,h,w]->[b, p^2, hw/p^2]
            img_per_ch = img_per_ch.view(b, p * p, h // p, w // p) # ->[b, p^2, h/p, w/p]
            img_per_ch = img_per_ch.permute(0, 2, 3, 1).reshape(-1) # ->[b, h/p, w/p, p^2] ->[bhw]
            img_per_ch = img_per_ch[masks[i]].reshape(b, h // p, w // p, 1) # [bhw]->[bhw/p^2]->[b, h/p, w/p, 1]
    ...
    noise_sup = noise_sup.repeat(1, (p*p-nss), 1, 1, 1) # [b, (p^2-n*s^2)*n, c, sh/p, sw/p]
...
if __name__ == '__main__':
    ...
    model = DBSNl(ids=args.ids).cuda() if not args.dist else \
        DDP(DBSNl(ids=args.ids).cuda(), [torch.cuda.current_device()])
