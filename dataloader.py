...
def read_img(filename): # filename:str
	...
	img = img[:, :, ::-1] / 255. # (H,W,C)f32[0,1]
	return np.array(img).astype('float32') # (H,W,C)f32[0,1]
...
def get_patch(img, ps): # img:(H,W,C)f32  ps:int
    ...
    img = img[yy:yy+ps_temp, xx:xx+ps_temp] # (ps,ps,C)f32[0,1]
...
class DatasetForTraining(Dataset):
    def __init__(self, noise_imgs, patch_sz):
        ...
        self.noise_imgs = noise_imgs#[:4]
    
    def __len__(self):
        return len(self.noise_imgs)

    def __getitem__(self, index):
        img = read_img(self.noise_imgs[index]) # (H,W,C)f32[0,1]
        img = get_patch(img, self.patch_sz) # (ps,ps,C)f32[0,1]
...
class SIDD_Ben_folder(Dataset):
    def __init__(self, noise_imgs, saved_imgs):
        super().__init__()
        self.noise_imgs = noise_imgs#[:3]
        self.saved_imgs = saved_imgs#[:3]
        self.len = len(self.noise_imgs)
